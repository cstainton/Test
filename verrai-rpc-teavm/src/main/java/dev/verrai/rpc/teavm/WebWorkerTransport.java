...
public class WebWorkerTransport implements Transport {
    ...
    public WebWorkerTransport(JSObject worker) {
        this.worker = worker;
        listenToWorker(worker, this::handleMessage);
    }
    ...
    public interface MessageHandlerCallback extends JSObject {
        void handle(MessageEvent event);
    }

    @JSBody(params = { "target", "callback" }, script = "target.onmessage = function(e) { callback(e); };")
    private static native void listenToWorker(JSObject target, MessageHandlerCallback callback);
